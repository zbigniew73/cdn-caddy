(function () {
  ...
  var DICT = {
    pl: {
      ...
      caddy_step2_title: 'Krok 2: wdroz konfiguracje CDN',
      caddy_step2_hint: 'Kopiuje wystawiony certyfikat do /etc/caddy/certs/ i zapisuje /etc/caddy/sites/{domain}.caddy (jeden katalog /var/www/cdn, routing po tokenie w sciezce) - domena wziete z kafelka "Pula CDN". Wymaga wczesniej wystawionego certu (zakladka Gcore DNS -> Certyfikaty TLS).',
      caddy_step2_btn: 'Wdroz konfiguracje CDN',
      caddy_step_running: 'Sprawdzanie...',
      caddy_step_success: 'Sprawdzono - wszystko OK',
      caddy_step_failed: 'Blad',
      info_tile_title: 'Info',
      info_tile_placeholder: 'Do zbudowania w kolejnym kroku.',
      pop_servers_title: 'Serwery POP',
      pop_servers_hint: 'Rejestr maszyn POP do wdrazania/synchronizacji configu Caddy przez SSH (klucz z wymuszonym poleceniem) - niezalezny od GeoDNS wyzej.',
      pop_servers_empty: 'Brak dodanych serwerow POP.',
      pop_servers_sync_btn: 'Synchronizacja',
      pop_servers_sync_running: 'Synchronizowanie...',
      th_pop_sync_status: 'Status synchronizacji',
      pop_servers_not_synced: 'Niezsynchronizowany',
      pop_servers_add_title: 'Dodaj serwer POP',
      pop_servers_host_label: 'Host',
      pop_servers_add_btn: 'Dodaj serwer',
      pop_servers_delete_confirm: 'Usunac serwer POP "{host}" z rejestru?',
      pop_servers_deploy_title: 'Jednorazowa konfiguracja dostepu SSH na nowym POP-ie',
      pop_servers_deploy_hint: 'Zanim klikniesz "Synchronizacja" dla nowego serwera POP, recznie (jako root, po SSH) wykonaj tam ten jeden raz: zapisz ponizszy skrypt jako {path}, nadaj mu prawo wykonywania (chmod +x), a nastepnie dopisz ponizsza linie do /root/.ssh/authorized_keys. Klucz panelu bedzie mogl WYLACZNIE uruchamiac ten skrypt (wymuszone polecenie "command=") - to "czesciowa kontrola", nie pelny dostep roota.',
      pop_servers_deploy_script_label: 'Tresc skryptu (zapisz jako plik na POP-ie)',
      pop_servers_deploy_key_label: 'Linia do /root/.ssh/authorized_keys na POP-ie',
    ...
    en: {
      ...
      caddy_step2_title: 'Step 2: deploy CDN configuration',
      caddy_step2_hint: 'Copies the issued certificate into /etc/caddy/certs/ and writes /etc/caddy/sites/{domain}.caddy (single /var/www/cdn directory, path-based token routing) - domain taken from the "CDN Pool" tile. Requires a certificate issued beforehand (Gcore DNS tab -> TLS Certificates).',
      caddy_step2_btn: 'Deploy CDN configuration',
      caddy_step_running: 'Checking...',
      caddy_step_success: 'Checked - all OK',
      caddy_step_failed: 'Error',
      info_tile_title: 'Info',
      info_tile_placeholder: 'To be built in the next step.',
      pop_servers_title: 'POP Servers',
      pop_servers_hint: 'Registry of POP machines for Caddy config deployment/sync over SSH (forced-command key) - independent from GeoDNS above.',
      pop_servers_empty: 'No POP servers added yet.',
      pop_servers_sync_btn: 'Sync',
      pop_servers_sync_running: 'Syncing...',
      th_pop_sync_status: 'Sync status',
      pop_servers_not_synced: 'Not synced yet',
      pop_servers_add_title: 'Add POP server',
      pop_servers_host_label: 'Host',
      pop_servers_add_btn: 'Add server',
      pop_servers_delete_confirm: 'Delete POP server "{host}" from the registry?',
      pop_servers_deploy_title: 'One-time SSH access setup on a new POP',
      pop_servers_deploy_hint: 'Before clicking "Sync" for a new POP server, do this once manually there (as root, over SSH): save the script below as {path}, make it executable (chmod +x), then append the line below to /root/.ssh/authorized_keys. The panel key will only ever be able to run this one script (forced "command=") - this is "partial control", not full root access.',
      pop_servers_deploy_script_label: 'Script contents (save as a file on the POP)',
      pop_servers_deploy_key_label: 'Line for /root/.ssh/authorized_keys on the POP',
